 ...
 enumerable_3 = {"a"=> 2, "b"=> 3, "c"=> 4}
  describe "#my_each" do
    ...
    it "returns each element being added by five" do
      hash = []
      enumerable_3.my_each {|el, value| hash << value + 5}
  ...
  describe "#my_select" do
    it "returns each element that is divisible by two." do
      arr = []
      enumerable.my_select {|el| arr << el if el % 2 == 0}
      expect(arr).to eq([2])
    end
  ...
  describe "#my_map" do
    ...
    it "returns each element in symbol form." do
      hash = []
      enumerable_3.my_map{|el, value| hash << el.to_sym}
  ...
  describe "#my_any?" do
    it "checks if any elements are even." do
      flag = enumerable.my_any?{|el| el % 2 == 0}
      expect(flag).to eq(true)
    end

    it "checks if any elements are odd." do
      flag = enumerable.my_any?{|el| el.odd?}
      expect(flag).to eq(true)
    end
    ...
    it "checks if any elements are string type." do
      flag = enumerable_3.my_any?{|el, value| el.class == String}
      expect(flag).to eq(true)
    end

    it "checks if any element's values are string type." do
      flag = enumerable_3.my_any?{|el, value| value.class == String}
      expect(flag).to eq(false)
    end
  end  
  ...
  describe "#my_none?" do
    ...
    it "checks if any elements are of the nil type." do
      flag = enumerable_2.my_none?{|el| el.nil?}
      expect(flag).to eq(true)
    end
  ...
  describe "#my_count" do
    ...
    it "checks how many elements there are that are divisible by 2." do
      number = enumerable_2.my_count{|el| el %2 == 0}
      expect(number).to eq(1)
    end
  end
